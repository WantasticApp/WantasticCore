...
type TenantProxy struct {
	...
	// setCopilotAPIKey is wired by the portal at startup. When non-nil the
	// CopilotService.SetAPIKey WS endpoint forwards admin-supplied keys to
	// it so the host can persist them (e.g. config.yaml) and re-initialize
	// the live Copilot LLM without a process restart.
	setCopilotAPIKey func(apiKey string) error
...
// SetCopilotAPIKeyHandler wires the persistence callback invoked when an
// admin sets a new Anthropic API key from the in-app Copilot setup form.
// Optional — when nil, the SetAPIKey WS endpoint reports unsupported.
func (p *TenantProxy) SetCopilotAPIKeyHandler(fn func(apiKey string) error) {
	p.setCopilotAPIKey = fn
}
