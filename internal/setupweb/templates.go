...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>WantasticCore — Setup complete</title>
  <style>
    :root { color-scheme: light dark; }
    body {
      font-family: ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif;
      max-width: 780px; margin: 0 auto; padding: 2rem 1.25rem; line-height: 1.5;
      background: #0f1218; color: #e5e7eb;
    }
    h1 { margin: 0 0 0.5rem; font-size: 1.6rem; color: #34d399; }
...
<body>
  ...
  <p class="lede">Configuration written to <code>/etc/wantastic/config.yaml</code>.
    The service will restart in a moment with the new config.</p>
  ...
  <div class="card">
    <h2 style="margin-top:0;font-size:1.1rem;">What happens next</h2>
    <ol class="next-steps">
      <li>Add the DNS records above with your registrar.</li>
      <li>Wait a few minutes for propagation.</li>
      <li>Visit <a href="https://{{ .Console }}/" style="color:#60a5fa;">https://{{ .Console }}/</a> and log in with the super-admin email and password you just set.</li>
      <li>From the portal, create tenant accounts under <em>Admin → Users</em>.</li>
    </ol>
  </div>

  <p class="lede" style="font-size:0.85rem;">You can close this window. The
    setup listener will shut down automatically.</p>
</body>
</html>
`))
