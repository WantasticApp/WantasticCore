...
func (device *Device) NewOutboundElement() *QueueOutboundElement {
	...
	elem.nonce = 0
	elem.msgType = 0 // Reset to default (MessageTransportType) — prevents stale WUSP/stats types from pool reuse
...
func (peer *Peer) SendTUNControl(data []byte) {
	...
	case peer.queue.staged <- elemsContainer:
		peer.device.log.Verbosef("%v - Sending TUN control message", peer)
	default:
...
top:
	...
	if keypair == nil || keypair.sendNonce.Load() >= RejectAfterMessages || time.Since(keypair.created) >= RejectAfterTime {
		peer.device.log.Errorf("%v - SendStagedPackets: no valid keypair (staged=%d), initiating handshake", peer, len(peer.queue.staged))
		peer.SendHandshakeInitiation(false)
		return
	}
