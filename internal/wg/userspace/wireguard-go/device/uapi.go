...
import (
	...
	"WantasticCore/internal/wg/userspace/wireguard-go/ipc"
)
...
func (device *Device) IpcGetOperation(w io.Writer) error {
	...
	func() {
		...
		for _, peer := range device.peers.keyMap {
			...
			authNano := peer.lastAuthenticatedPacketNano.Load()
			authSecs := authNano / time.Second.Nanoseconds()
			authNano %= time.Second.Nanoseconds()
			sendf("last_authenticated_packet_time_sec=%d", authSecs)
			sendf("last_authenticated_packet_time_nsec=%d", authNano)
