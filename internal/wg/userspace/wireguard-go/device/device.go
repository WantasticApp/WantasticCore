...
import (
	...
	"WantasticCore/internal/wg/userspace/wireguard-go/ratelimiter"
...
type Device struct {
	...
	PeerAnnounceHandler func(*NoisePublicKey)
	...
	// P2P components
	coordinator *Coordinator // Server only
	p2pClient   *P2PClient   // Client only

	tunControlHandler           func(*Peer, []byte) // Handler for P2P TUN mode coordination (message type 7)
	wuspHandler                 func(*Peer, []byte) // Handler for WUSP device management (message type 8)
	peerSessionConfirmedHandler func(*Peer)         // fires once per new WireGuard session when ReceivedWithKeypair returns true
}

func (device *Device) EnableCoordinator() {
	device.coordinator = NewCoordinator(device)
}

// GetCoordinator returns the coordinator instance (if enabled)
func (device *Device) GetCoordinator() *Coordinator {
	return device.coordinator
}

func (device *Device) EnableP2PClient() {
	device.p2pClient = NewP2PClient(device)
	device.p2pClient.Start()
}

func (device *Device) SetPeerAnnounceHandler(handler func(*NoisePublicKey)) {
	device.PeerAnnounceHandler = handler
}
...
// WireGuard session, when ReceivedWithKeypair returns true (both sides have
// confirmed the new keypair). This is the earliest safe point to send WUSP
// probes, because the controller's keypairs.Current() is now the new session key.
func (device *Device) SetPeerSessionConfirmedHandler(handler func(*Peer)) {
	device.peerSessionConfirmedHandler = handler
}
...
func (peer *Peer) SendWUSP(data []byte) error {
	...
	case peer.queue.staged <- elemsContainer:
		peer.device.log.Verbosef("%v - Sending WUSP control message", peer)
	default:
