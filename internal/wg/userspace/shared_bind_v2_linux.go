...
// 40% rate from net link max queue size (128k)
package userspace
...
const (
	...
	// for handshake/cookie/P2P traffic. Control packets are rare (a few per
	// peer per minute) so 512 is well over the worst-case burst, but the
	// lane is small enough that a stuck consumer stops accepting control
	// packets quickly and emits a queue-full warning instead of silently
	// hiding behind the much larger data lane.
...
type tenantRoute struct {
	tenantID string
	// recvCh carries data-plane traffic (Transport/Stats/TUNControl/WUSP).
	// controlCh is a small priority lane for handshake/cookie/P2P packets
	// so a fresh handshake can still reach WireGuard while the data lane is
	// full — preventing the "online but unable to reconnect" failure mode.
...
const (
	...
	handshakeTokensBucket = 100 // Max tokens (burst)
	handshakeTokensRate   = 10  // Tokens per 100ms
...
func (b *SharedPortBindV2) dispatchPacket(data []byte, addr netip.AddrPort) {
	...
	// 	packet_type := map[uint8]string{
	// 		messageTypeHandshakeInitiation: "HandshakeInitiation",
	// 		messageTypeHandshakeResponse:   "HandshakeResponse",
	// 		messageTypeTransportData:       "TransportData",
	// 		messageTypeStats:               "Stats",
	// 	}
	// 	switch data[0] {
	// 	case messageTypeHandshakeInitiation, messageTypeHandshakeResponse, messageTypeTransportData, messageTypeStats:
	// 		log.Debug().
	// 			Str("src", addr.String()).
	// 			Int("len", len(data)).
	// 			Str("type", packet_type[data[0]]).
	// 			Msg("SharedBind RX")
	// 	}
	// }
	msgType := data[0]
...
func (b *SharedPortBindV2) broadcastHandshake(data []byte, addr netip.AddrPort) {
	...
	b.handshakeBroadcast.Range(func(key, value any) bool {
		...
		default:
			...
			log.Warn().Str("tenant", route.tenantID).Msg("Handshake broadcast: tenant queue full")
		}
		return true
	})

	if broadcastCount == 0 {
		log.Warn().Msg("Handshake broadcast: NO TENANTS REGISTERED")
	} else {
		log.Debug().Int("count", broadcastCount).Msg("Handshake broadcast completed")
	}
}
...
// (handshake init/response, cookie reply, P2P) goes onto the small priority
// lane so a fresh handshake survives even when the data lane is full —
// this is what lets a peer reconnect through a temporarily-saturated tenant.
func (b *SharedPortBindV2) deliverToTenant(route *tenantRoute, data []byte, addr netip.AddrPort) {
	...
	default:
		...
		// and the peer just looks "online but unreachable"). When this
		// fires for the control lane it is significant: handshakes can
		// no longer reach WireGuard, and the peer cannot reconnect.
...
func (b *SharedPortBindV2) UnregisterTenant(tenantID string) {
	...
	// Values are *addrRouteCacheEntry (wrapping *tenantRoute), not *tenantRoute directly.
...
func (b *perTenantBindV2) Open(port uint16) ([]conn.ReceiveFunc, uint16, error) {
	...
	// Create receive function. Priority pass drains the control lane first
	// (handshake/cookie/P2P) so a fresh handshake reaches WireGuard even
	// while the data lane is saturated. A nil controlCh is harmless: a nil
	// channel case in select never fires.
	receiveFn := func(bufs [][]byte, sizes []int, eps []conn.Endpoint) (int, error) {
		for {
			...
			// Blocking select across both lanes plus close.
			select {
			case <-b.closeCh:
				return 0, net.ErrClosed
			case pkt := <-b.controlCh:
		...
		if len(buf) >= 8 && buf[0] == messageTypeHandshakeResponse {
			...
			if b.shared.peerActiveHandler != nil {
				...
				if dst != "" {
					...
					// However, we want to start it immediately.
					// Use a small buffer or go routine?
					// Since this is Handshake Response (happens once every few mins per peer),
					// spawning a goroutine is acceptable overhead.
