...
func StartWantasticCore(cfg WantasticCoreConfig) *error {
	...
	if cfg.TenantEnabled {
		...
		// Start inactivity cron for free-account reminders.
		// Sends the first reminder after 30 days, then follow-up reminders after 45+ days.
		if grpcConfig.SMTPClient != nil {
			...
			// Set up hook cipher for unsubscribe links if configured
			// Set up shared secret key
			var secretKey []byte
			var err error
			if cfg.HooksSecretKey != "" {
				secretKey, err = hex.DecodeString(cfg.HooksSecretKey)
				if err != nil {
				...
				// Use a static secret for development/testing so features aren't broken
				secretKey = []byte("wantastic-fallback-secret-for-dev-only")
			}

			// Initialize Enrollment Token Cipher if key is available
			if len(secretKey) > 0 {
				enrollmentCipher, err := crypto.NewEnrollmentTokenCipher(secretKey)
				if err != nil {
