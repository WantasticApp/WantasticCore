...
// and pass it into the WebSocket dispatcher.
type Service struct {
	cfg      Config
	llm      LLM
...
// New creates a Service. The Claude client is required (errors when it
// isn't Enabled() — Copilot only makes sense with an LLM). The admin
// service is optional; without it, admin-only tools fail closed.
func New(cfg Config, llm LLM, adminSvc *admin.Service, services *core.Services) (*Service, error) {
	if llm == nil {
		return nil, errors.New("copilot: llm is required")
	}
	if !llm.Enabled() {
		return nil, errors.New("copilot: llm not configured (claude api key missing)")
	}
	s := &Service{
		cfg:      cfg,
		llm:      llm,
...
func (s *Service) SendMessage(ctx context.Context, sessionID, userText string) (string, error) {
	...
	reply, err := s.llm.Chat(ctx, systemPrompt, histCopy, tools, dispatch)
	if err != nil {
		return "", err
	}
